...
```python seeing_soundboard_main.py --train --data-path [data directory] --model-path [directory where trained model will be saved]```  
  
  To use your trained model, use this command:  
  ```python seeing_soundboard_main.py --use --model-path [path to trained model, which should be a .task file]```